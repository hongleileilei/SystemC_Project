...
void PROCESS::updata_helper(float &robot_x, float &robot_y, int &direction, int &robot_before_boundary_0_5, int &now_grid, int &next_grid, int &if_ok_to_cross, int &status, vector<int> &vec, int &arrive_at_destination, int &lock2, int &stop_temp, int &return_value) {
    cout <<"ifoktocross: " << if_ok_to_cross << endl;
    ...
    if (arrive_at_destination == 0) { // 还没有到达path的终点
        ...
        if ( pow( abs(obstacle_x_1 - robot_x), 2 ) + pow( abs(obstacle_y_1 - robot_y), 2) > 9 && pow( abs(obstacle_x_2 - robot_x), 2 ) + pow( abs( obstacle_y_2 - robot_y ), 2 ) > 9 )  {
            ...
            cout << sc_time_stamp() << ", :direction: " << direction << ", returnvalue: " <<return_value << ", beforboundary0.5: " << robot_before_boundary_0_5 << ", " << ", ofoktocross: " << if_ok_to_cross << " ///////////////////////////" << endl;
            ...
            if ( robot_before_boundary_0_5 == 0 && lock2 == 0) { // 距离不等于0.5, 不用等待server信号
                ...
                cout << sc_time_stamp() << ", " << if_ok_to_cross << ", ssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss" << endl;
...
void PROCESS::updata() {
    ...
    cout << "ifoktocors: " << if_ok_to_cross_1 << ", kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk" << endl;
    ...
    cout << "   cg1: " << now_grid_1 <<  ", ng1: " <<next_grid_1 << ", cg2: " << now_grid_2 << ", ng2: " << next_grid_2 << ", x1: " << robot_x_1 << ", y1: " << robot_y_1 << ", x2: " << robot_x_2 << ", y2: " << robot_y_2 << endl; 
    cout << "   ox1: " << obstacle_x_1 << ", oy1: " << obstacle_y_1 << ", " << ", ox2: " << obstacle_x_2 << ", oy2: " << obstacle_y_2 << endl; 
}
...
void PROCESS::transmit_1() {
    ...
    cout << Next_grid_1.read() << endl;
    cout << sc_time_stamp() << ", process transmit (*(*(*(*(*(*(*(*(*(*(*(*(*(*(*(*(*(*(*(*(*(*(*(" << endl;
}

void PROCESS::receive_1() {
    if_ok_to_cross_1 = If_ok_to_cross_1.read(); // 1:可以通过， 0:不可以通过
    cout << if_ok_to_cross_1 << ", process receive +++++++++++++++++++++++++++++++++++++++++" << endl;
}
