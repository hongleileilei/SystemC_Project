...
SC_MODULE(SERVER) {
public:
    sc_in<int> Trigger_in_1,Trigger_in_2;   // receive对trigger信号电平敏感
    ...
    sc_out<int> Trigger_out_1, Trigger_out_2;
    ...
    int map[2][3];
    int (*pt)[3] = NULL;
    int trigger_out_1, trigger_out_2;
    ...
    SERVER(sc_module_name server, int (*coeffs)[3], vector<int> path_1, vector<int> path_2):sc_module(server), pt(coeffs), vec_1(path_1), vec_2(path_2) {
        ...
        sensitive << Flag;
        SC_METHOD(receive);
        sensitive << Trigger_in_1 << Trigger_in_2;
    }
};
