...
void CTRL::counter(){
  counter_instr = Instr.read();
  counter_psr = psr.read();
  counter_z = counter_psr.range(3,3);
  counter_f = counter_psr.range(3,3);
  counter_n = counter_psr.range(3,3);
  counter_c = counter_psr.range(3,3);
...
void CTRL::proc(){
  ...
  cout<<lcl_instr<<" is the instructions we read\n";
  psr_val = psr.read();
