...
int PROCESS::if_reach_boundary(int &x, int &y, int &robot_before_boundary_0_5, int &direction, int &now_grid, int &next_grid) {  // 0:大于或小于0.5m; 1:等于0.5m了; 2:now_grid == next_grid
    ...
    if (now_grid == next_grid) { // 在结束时， 如果现在就在next_grid， 则不会发生边缘检测，需要保证robot能够停在grid的正中心。
        ...
        for (int direction = 4; direction < 8; direction++) {  // 与 next_grid 比较， 学找出robot的运动方向 direction
...
void PROCESS::updata_helper(int &robot_x, int &robot_y, int &direction, int &robot_before_boundary_0_5, int &now_grid, int &next_grid, int &if_ok_to_cross, int &status, vector<int> &vec, int &arrive_at_destination) {
            ...
            if (robot_before_boundary_0_5 == 0) { // 还没有到达0.5, 不用等待server信号
                if ( 2 == return_value ) { // 说明此时robot就在目标
                    if (direction == 4) { // 北方向
                        ...
                        if ( floor(robot_y) == robot_y + 1 ) { // robot到达grid中间
                            if (vec.size() != 0) {
                        ...
                        proc_robot_position(robot_x, robot_y, direction);
                        if (floor(robot_x) == robot_x + 1) { // robot到达grid中间
                            if (vec.size() != 0) {
                                vec.erase(vec.begin());//传送下一个grid
                            }
                            else {
...
void PROCESS::updata() {
    ...
    updata_helper(robot_x_1, robot_y_1, direction_1, robot_1_before_boundary_0_5, now_grid_1, next_grid_1, if_ok_to_cross_1, r1_status, vec_1, arrive_at_destination_1);
    updata_helper(robot_x_2, robot_y_2, direction_2, robot_2_before_boundary_0_5, now_grid_2, next_grid_2, if_ok_to_cross_2, r2_status, vec_2, arrive_at_destination_2);
