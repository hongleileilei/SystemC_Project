...
void ROBOT::transmit_server() {
    ...
    cout << sc_time_stamp() << ",   Robot transmit to server" << endl;
}

void ROBOT::receive_process() {
    cout << sc_time_stamp() << ",   Robot receive form process" << endl;
...
void ROBOT::transmit_process() {
    ...
    cout << sc_time_stamp() << ",   Robot transmit to process, ACK value: " << Ack_out.read() << endl;
}
void ROBOT::receive_server() {
    flag_transmit_process = 1 - flag_transmit_process;
    cout << sc_time_stamp() << ",   Robot recevive from server " << endl;
}